    ...
    <div className="min-h-screen bg-[#f7f8fa]" style={{ paddingTop: "var(--navbar-height, 60px)" }}>
      ...
      <main className="max-w-[1200px] mx-auto px-4 md:px-6 py-6 space-y-8">
        ...
        <nav className="flex items-center gap-1.5 text-sm text-gray-500">
          ...
          <span className="text-[#e85d04] font-medium">Reference Lookup Tool</span>
        </nav>

        {/* ── Page header: title + trust badges ────────── */}
        <div className="flex items-start justify-between gap-6 flex-wrap">
          <div>
            <h1 className="text-3xl md:text-4xl font-extrabold text-[#0d1b2a] leading-tight">
              Official Reference
              <br />
              <span className="text-[#e85d04]">Lookup Tool</span>
            </h1>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {BADGES.map(({ icon: Icon, label, sub }) => (
              <div key={label} className="flex items-center gap-2.5 bg-white border border-gray-100 rounded-2xl px-4 py-3 shadow-sm">
                <Icon size={24} className="text-[#e85d04] flex-shrink-0" />
                <div>
                  <p className="text-xs font-bold text-[#0d1b2a] leading-none">{label}</p>
        ...
        <div className="relative">
          ...
          <div className="relative md:absolute md:bottom-[-56px] md:left-1/2 md:-translate-x-1/2 md:w-[640px] mx-4 md:mx-0 mt-4 md:mt-0 bg-white rounded-3xl shadow-2xl border border-gray-100 p-6">
            <p className="text-sm font-bold text-[#0d1b2a] mb-4 text-center">
              Enter details to look up reference value
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
              <select className="border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-600 bg-white focus:outline-none focus:ring-2 focus:ring-[#e85d04]/30 focus:border-[#e85d04] appearance-none cursor-pointer">
            ...
            <button className="w-full bg-[#e85d04] hover:bg-[#cf4f02] text-white font-bold py-3.5 rounded-xl text-sm transition-colors flex items-center justify-center gap-2">
        ...
        <section className="pt-4">
          <h2 className="text-xl font-extrabold text-[#0d1b2a] mb-6">How It Works</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {STEPS.map(({ num, title, desc }) => (
              <div key={num} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <span className="inline-block text-3xl font-extrabold text-[#e85d04]/20 mb-3 leading-none">
                  {num}
                </span>
        ...
        <section className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 md:p-8">
          <h2 className="text-xl font-extrabold text-[#0d1b2a] mb-6">Why Use This Tool?</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {FEATURES.map(({ title, desc }) => (
              <div key={title} className="space-y-2">
                <CheckCircle2 size={22} className="text-[#e85d04]" />
